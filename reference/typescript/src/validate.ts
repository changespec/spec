import { z } from "zod";
import { EventSchema, ParsedEvent, isExtKey } from "./types.js";

// ValidationResult is a discriminated union returned by validate().
export type ValidationResult =
  | { success: true; data: ParsedEvent }
...
export function validate(raw: unknown): ValidationResult {
  ...
  const result = EventSchema.safeParse(coreInput);
  ...
  const event: ParsedEvent = {
    ...(result.data as Omit<ParsedEvent, "extensions">),
    extensions,
  };
 ...
 */
export function validateOrThrow(raw: unknown): ParsedEvent {
